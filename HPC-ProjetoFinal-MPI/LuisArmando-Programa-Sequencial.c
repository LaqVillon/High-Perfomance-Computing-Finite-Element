...
static double Mvelha[768][768];
static double Mnova[768][768];
...
{ 
    ...
    while ( precisao < erro )
    {
		...
		for(j = Ny; j <=l3 + 2; j++)
		{
		    Mvelha[l2][j] = Mvelha[Nx+1][j];
		    Mnova[l2][j] = Mnova[Nx+1][j];
		}
		
		//----------------------------------------Contorno 5.
		for(j = 0; j<=maxy - 1; j++)
		{
		    Mvelha[l4+1][j] = c8*Mvelha[l7+1][j]+c7;
		    Mnova[l4+1][j] = c8*Mvelha[l7+1][j]+c7;
		}
